...
CSV_PATH = os.path.join("raw_data", "locationcategories.csv")
...
while retry:
    ...
    for i in range(begin // 1000, (total_count // 1000) + 1):
        ...
        try:
            ...
            break
        for x in data:
            if x not in locationcategories:
                locationcategories.append(x)
        print(f"offset={offset}\tlimit={limit}\tcount={len(locationcategories)}")
        last_offset = offset

    if not err:
        retry = False

df = pd.DataFrame(locationcategories)
